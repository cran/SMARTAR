...
tumat=function(sim,family="normal"){
      Smat=sim; FA=family
      ...
      if (Nstage==2 & Base==0) {
          for (g in 1:G){ATS=as.numeric(Dmat[g,2:4])
                         ATS[which(is.na(ATS))]=0
                         S0=Smat[which(Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[2]),] #seq (d1,O2=0,d20)
                         S1=Smat[which(Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[3]),] #seq (d1,O2=1.d21)
                         u0=S0$MEAN; if (length(u0)==0L) {u0=0}
      ...
      if (Nstage==2 & Base==1) {
          for (g in 1:G){ATS=as.numeric(Dmat[g,2:8])
                         ATS[which(is.na(ATS))]=0
                         S00=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[3]),]
                         S01=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[4]),]
                         S10=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==0 & Smat$A2==ATS[5]),]
                         S11=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==1 & Smat$A2==ATS[6]),]
      ...
      if (Nstage==3 & Base==0) {
         for (g in 1:G){ATS=as.numeric(Dmat[g,2:8])
                        ATS[which(is.na(ATS))]=0
                        S00=Smat[which(Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[2] & Smat$O3==0 & Smat$A3==ATS[4]),]
                        S01=Smat[which(Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[2] & Smat$O3==1 & Smat$A3==ATS[5]),]
                        S10=Smat[which(Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[3] & Smat$O3==0 & Smat$A3==ATS[6]),]
                        S11=Smat[which(Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[3] & Smat$O3==1 & Smat$A3==ATS[7]),]
                        u00=S00$MEAN; if (length(u00)==0L) {u00=0}
                        u01=S01$MEAN; if (length(u01)==0L) {u01=0}
                        u10=S10$MEAN; if (length(u10)==0L) {u10=0}
                        u11=S11$MEAN; if (length(u11)==0L) {u11=0}
      ...
      if (Nstage==3 & Base==1) {
          for (g in 1:G){ATS=as.numeric(Dmat[g,2:15])
                         ATS[which(is.na(ATS))]=0
                         S000=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[3] & Smat$O3==0 & Smat$A3==ATS[7]),]
                         S001=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==0 & Smat$A2==ATS[3] & Smat$O3==1 & Smat$A3==ATS[8]),]
                         S010=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[4] & Smat$O3==0 & Smat$A3==ATS[9]),]
                         S011=Smat[which(Smat$O1==0 & Smat$A1==ATS[1] & Smat$O2==1 & Smat$A2==ATS[4] & Smat$O3==1 & Smat$A3==ATS[10]),]
                         S100=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==0 & Smat$A2==ATS[5] & Smat$O3==0 & Smat$A3==ATS[11]),]
                         S101=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==0 & Smat$A2==ATS[5] & Smat$O3==1 & Smat$A3==ATS[12]),]
                         S110=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==1 & Smat$A2==ATS[6] & Smat$O3==0 & Smat$A3==ATS[13]),]
                         S111=Smat[which(Smat$O1==1 & Smat$A1==ATS[2] & Smat$O2==1 & Smat$A2==ATS[6] & Smat$O3==1 & Smat$A3==ATS[14]),]
